...
import { ListItem, ListItemAvatar, Avatar, ListItemText, Divider, IconButton, ListItemSecondaryAction, Badge } from '@material-ui/core';
import DeleteIcon from '@material-ui/icons/Delete';
import ThumbUpIcon from '@material-ui/icons/ThumbUp';

import { mutationForDeleteUser, mutationForAddLike } from './UserItem.mutations';
...
const UserItem = ({ user }: { user: IUser }) => {
    ...
    const onAddLike = async (id: string, likesBeforeClick:number) => {
        await addLike({
            variables: { id },
            optimisticResponse:{
                __typename: 'Mutation',
        ...
        <div key={user.id}>
            <ListItem button onClick={() => { history.push(`/users/${user.id}`) }}>
                ...
                <ListItemSecondaryAction>
                    <IconButton edge="end" aria-label="like" onClick={() => { onAddLike(user.id, user.likes) }}>
                        <Badge
